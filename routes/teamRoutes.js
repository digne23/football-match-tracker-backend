...
router.delete('/:id', protect, async(req, res) => {
    try {
        ...
        const team = await Team.findOneAndDelete({ _id: teamId, createdBy: req.user.id })
...
router.get('/:id', protect, async (req, res) => {
    try {
        const team = await Team.findOne({ _id: req.params.id, createdBy: req.user_id })
    
        if (!team)
            return res.status(401).json({ message: "no corresponding team was found" })
        res.json(team)
    } catch (error) {
        res.status(500).json({message:"server error"})
    }
